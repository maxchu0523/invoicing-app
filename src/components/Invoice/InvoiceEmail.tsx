...
Invoice Items:
${invoice.invoiceItems.map((item) => `${item.description}: ${item.quantity} x ${item.rate} = ${item.quantity * item.rate}`).join('\n')}
...
Max`;
    ...
    const handleComposeEmail = () => {
        ...
        window.location.href = `mailto:${invoice?.billingAddress}?subject=${subject}&body=${encodeURIComponent(emailContent)}`;
    };
